...
class DOAJXmlFilter extends NativeExportFilter {
	...
	function &process(&$pubObjects) {
		...
		foreach($pubObjects as $pubObject) {
			...
			// Language
			$language = AppLocale::get3LetterIsoFromLocale($pubObject->getLocale('en_US'));
			...
			// documentType
			$type = $pubObject->getType($pubObject->getLocale('en_US'));
			...
			if (array_key_exists($pubObject->getLocale('en_US'), $articleTitles)) {
				$titleInArticleLocale = $articleTitles[$pubObject->getLocale('en_US')];
				unset($articleTitles[$pubObject->getLocale('en_US')]);
				$articleTitles = array_merge(array($pubObject->getLocale('en_US') => $titleInArticleLocale), $articleTitles);
			}
			...
			if (array_key_exists($pubObject->getLocale('en_US'), $articleAbstracts)) {
				$abstractInArticleLocale = $articleAbstracts[$pubObject->getLocale('en_US')];
				unset($articleAbstracts[$pubObject->getLocale('en_US')]);
				$articleAbstracts = array_merge(array($pubObject->getLocale('en_US') => $abstractInArticleLocale), $articleAbstracts);
			}
			...
			if (array_key_exists($pubObject->getLocale('en_US'), $articleKeywords)) {
				$keywordsInArticleLocale = $articleKeywords[$pubObject->getLocale('en_US')];
				unset($articleKeywords[$pubObject->getLocale('en_US')]);
				$articleKeywords = array_merge(array($pubObject->getLocale('en_US') => $keywordsInArticleLocale), $articleKeywords);
			}
	...
	function createAuthorNode($doc, $article, $author, $affilList) {
		...
		if(in_array($author->getAffiliation($article->getLocale('en_US')), $affilList)  && !empty($affilList[0])) {
			$authorNode->appendChild($node = $doc->createElement('affiliationId', htmlspecialchars(current(array_keys($affilList, $author->getAffiliation($article->getLocale('en_US')))), ENT_COMPAT, 'UTF-8')));
		}
		return $authorNode;
	}
	...
	function createAffiliationsList($authors, $article) {
		...
		foreach ($authors as $author) {
			if(!in_array($author->getAffiliation($article->getLocale('en_US')), $affilList)) {
				$affilList[] = $author->getAffiliation($article->getLocale('en_US')) ;
			}
		}
		return $affilList;
	}

...
class DOAJJsonFilter extends NativeImportExportFilter {
	...
	function &process(&$pubObject) {
		...
		// To-Do: license ???
		// Journal's title (M)
		$journalTitle =  $context->getName($context->getPrimaryLocale(en_US));
		...
		// Article title
		$article['bibjson']['title'] = $pubObject->getTitle('en_US');
		// Identifiers
		...
		foreach ($articleAuthors as $articleAuthor) {
			...
			$affiliation = $articleAuthor->getAffiliation($pubObject->getLocale('en_US'));
			if (!empty($affiliation)) $author['affiliation'] = $affiliation;
			$article['bibjson']['author'][] = $author;
		}
		// Abstract
		$abstract = $pubObject->getAbstract('en_US');
		if (!empty($abstract)) $article['bibjson']['abstract'] = PKPString::html2text($abstract);
		// Keywords
		$dao = DAORegistry::getDAO('SubmissionKeywordDAO');
		$keywords = $dao->getKeywords($pubObject->getId(), array($pubObject->getLocale('en_US')));
		$allowedNoOfKeywords = array_slice($keywords[$pubObject->getLocale('en_US')], 0, 6);
		if (!empty($keywords[$pubObject->getLocale('en_US')])) $article['bibjson']['keywords'] = $allowedNoOfKeywords;
